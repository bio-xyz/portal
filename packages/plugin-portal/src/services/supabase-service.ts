...
export class SupabaseService extends Service {
  ...
  async getUserLevel(privyId: string): Promise<UserLevel | null> {
    if (!privyId) {
      logger.warn('[SupabaseService] getUserLevel called without privyId');
      return null;
    }

    try {
      logger.info(`[SupabaseService] Getting user level for privyId: ${privyId}`);
      ...
      if (error) {
        logger.error(`[SupabaseService] Error fetching user level for ${privyId}:`, error);
      ...
      logger.info(`[SupabaseService] Fetched user level data for ${privyId}:`, data);
      ...
      logger.error(`[SupabaseService] Unexpected error in getUserLevel for ${privyId}:`, error);
   ...
   * @param privyId The user's privy ID
   * @returns The user level response
   */
  async createDefaultUserLevel(
    privyId: string
  ): Promise<{ level: number; updated_at: string } | null> {
    if (!privyId) {
      logger.error('[SupabaseService] createDefaultUserLevel requires privyId.');
      return null;
    }

    try {
      logger.info(`[SupabaseService] Creating default user level for privyId: ${privyId}`);

      const { data, error } = await supabase.rpc('create_default_user_level', {
        p_privy_id: privyId,
      });

      if (error) {
        logger.error(
          `[SupabaseService] Error creating default user level via RPC for ${privyId}:`,
          error
        );
        return null;
      }

      logger.info(`[SupabaseService] Successfully created default level for ${privyId} using RPC`);
      ...
      logger.error(`[SupabaseService] Error in createDefaultUserLevel for ${privyId}:`, error);
      return null;
    }
  }
   ...
   * @returns The user level data
   */
  async createOrUpdateUserLevel(privyId: string): Promise<UserLevel | null> {
    if (!privyId) {
      logger.warn('[SupabaseService] createOrUpdateUserLevel called without privyId');
      return null;
    }

    logger.info(
      `[SupabaseService] Ensuring user level record exists/is level 1 for privyId: ${privyId}`
    );
    ...
    try {
      ...
      if (error) {
        logger.error(`[SupabaseService] Error upserting user level for ${privyId}:`, error);
        if (error.code === '42501') {
      ...
      if (data && typeof data.level === 'number' && typeof data.privy_id === 'string') {
        ...
        logger.error(
          `[SupabaseService] Upsert completed for ${privyId} but returned unexpected data structure:`,
          data
        );
        return null;
      } else {
        logger.error(`[SupabaseService] Upsert completed for ${privyId} but no data returned.`);
        return null;
      }
    } catch (error) {
      logger.error(
        `[SupabaseService] Unexpected error in createOrUpdateUserLevel for ${privyId}:`,
        error
      );
      return null;
    }
  ...
  async updateUserLevel(
    ...
    if (!privyId) {
      logger.warn('[SupabaseService] updateUserLevel called without privyId');
    ...
    logger.info(`[SupabaseService] Updating user level for ${privyId} to ${newLevel}...`);
    ...
    try {
      ...
      if (error) {
        logger.error(`[SupabaseService] Error updating user level for ${privyId}:`, error);
        if (error.code === '42501') {
          logger.error('RLS Error: Check JWT claims and UPDATE policy on user_levels.');
        } else if (error.code === 'PGRST116') {
          logger.error('PGRST116 occurred unexpectedly during update for ', privyId);
        }
      ...
      logger.info(
        `[SupabaseService] User level update request sent for ${privyId} to level ${newLevel}.`
      );
      return { success: true };
    } catch (error) {
      logger.error(`[SupabaseService] Unexpected error in updateUserLevel for ${privyId}:`, error);
      return { success: false, error };
    }
  }
   ...
   * @returns The result of the operation
   */
  async incrementUserLevel(privyId: string): Promise<{ success: boolean; error?: any }> {
    try {
      // First get the current level object or null
      const currentUserLevelData = await this.getUserLevel(privyId);

      // Check if user level data exists
      if (!currentUserLevelData) {
        logger.error(
          `[SupabaseService] Cannot increment level for ${privyId}: User level data not found.`
        );
        return {
          success: false,
          error: new Error(`User level data not found for user ${privyId}`),
        };
      }
      ...
      return this.updateUserLevel(privyId, nextLevel);
    } catch (error) {
      logger.error(`[SupabaseService] Error incrementing level for ${privyId}:`, error);
      return { success: false, error };
    }
  }
  ...
  async checkRequirementsCompletion(
    ...
    try {
      logger.info(
        `[SupabaseService] Checking requirements completion for privyId: ${privyId} at level: ${level}`
      );
      ...
      logger.info(
        `[SupabaseService] Successfully retrieved requirements completion for user ${privyId} at level ${level}`
      );
      return completions;
    } catch (error) {
      logger.error(
        `[SupabaseService] Error checking requirements completion for ${privyId} at level ${level}:`,
        error
      );
      return [];
    }
   ...
   */
  async getUserMetrics(privyId: string): Promise<Record<string, any> | null> {
    try {
      logger.info(`[SupabaseService] Getting user metrics for privyId: ${privyId}`);

      // Call the Supabase RPC function to get user metrics
      const { data, error } = await this.rpc('get_user_metrics', { p_privy_id: privyId });
      ...
      if (!data) {
        logger.info(`[SupabaseService] No metrics found for user ${privyId}`);
        return null;
      }

      logger.info(`[SupabaseService] Successfully retrieved metrics for user ${privyId}`);
      return data as Record<string, any>;
    } catch (error) {
      logger.error(`[SupabaseService] Error getting user metrics for ${privyId}:`, error);
      return null;
    }
  }
}
