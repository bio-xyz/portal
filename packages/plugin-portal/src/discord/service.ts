...
export class DiscordService extends Service implements IDiscordService {
   ...
   * @returns {Promise<Guild | null>} The guild object if found, null otherwise
   */
  ...
  /**
   * Extract a Discord server ID from a message text
   * @param {string} text - The message text to extract the server ID from
   * @returns {Promise<string | null>} The extracted server ID if found, null otherwise
   */
  async extractServerIdFromMessage(text: string): Promise<string | null> {
    if (!text) return null;
