 ...
 */
async function mintSingleNFT(tokenId: bigint, embeddedWallet: ConnectedWallet, comment?: string): Promise<Hex> {
    const accountAddress = embeddedWallet.address as Hex;
    console.log(`[mintSingleNFT] Attempting to mint Token ID: ${tokenId} for ${accountAddress} using Privy EIP-1193 provider`);

    // Ensure wallet is on the correct chain
    if (embeddedWallet.chainId !== `eip155:${CHAIN.id}`) {
        console.error(`[mintSingleNFT] Wallet is on wrong chain: ${embeddedWallet.chainId}, expected eip155:${CHAIN.id}`);
    ...
    try {
        ...
        console.error(`[mintSingleNFT] Error sending mint transaction for token ${tokenId} via Viem/Privy:`, error);
 ...
 */
export async function mintIdeaAndVisionNFTs(embeddedWallet: ConnectedWallet): Promise<{ ideaNftHash: Hex; visionNftHash: Hex }> {
    const accountAddress = embeddedWallet.address as Hex;
    console.log(`[mintIdeaAndVisionNFTs] Starting mint process for ${accountAddress} using Privy Wallet`);
    ...
    try {
        ...
        ideaNftHash = await mintSingleNFT(IDEA_NFT_ID, embeddedWallet, 'Minting Idea NFT via BioDAO Portal');
        ...
        visionNftHash = await mintSingleNFT(VISION_NFT_ID, embeddedWallet, 'Minting Vision NFT via BioDAO Portal');
...
export async function waitForTransaction(hash: Hex) {
    ...
    try {
        ...
        if (error instanceof Error && error.message.includes('timed out')) {
            throw new Error(`Transaction confirmation timed out for ${hash}. It might still succeed, but check a block explorer.`);
        }
